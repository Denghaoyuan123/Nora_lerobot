...
class Nora:
    ...
    def __init__(
        ...
        self.model = VLAWithExpert(device='cpu')
    ...
    def inference(self, image: np.ndarray | PIL.Image.Image, instruction: str,
        ...
        unnorm_action = self.model.sample_action(image,instruction,num_steps=10,unnormalize=False)
        unnorm_action[:,-1:] = unnorm_action[:,-1:]*2-1
        
        output_action  = unnormalizer({'action':unnorm_action})
...
class InferenceEvaluator:
    ...
    def get_sample(self, idx: int) -> Tuple[PIL.Image.Image, np.ndarray, str]:
        """获取数据集样本"""
        ex = self.dataset[idx]
        img_chw = ex["observation.images.front"]
        action = ex["action"].float().numpy()
        lang = ex["task"]
        print(img_chw.shape)
        breakpoint()
        img_pil = to_pil_image(img_chw)
        return img_pil, action, lang
    ...
    def evaluate_batch(self, nora_model: Nora, unnormalizer, indices: List[int], 
                      map_gripper: bool = False, invert_gripper: bool = False) -> Dict:
        """批量评估"""
        batch_results = []
        batch_times = []
        ...
        for idx in tqdm(indices):
            try:
                # 获取样本
                img, gt_action, instruction = self.get_sample(idx)
...
def main():
    ...
    parser.add_argument("--model_path", type=str, 
                        default="/home/pine/Nora_lerobot/inference/so101_combined_sft_5k.safetensors")
    ...
    parser.add_argument("--lerobot_dataset_root", type=str, default="//home/pine/Nora_lerobot/inference/so101_pick_the_cube_ee")
    ...
    if args.mode == "single":
        ...
        img, gt_action, instruction = evaluator.get_sample(args.start_idx)
